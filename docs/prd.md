...
**Versión:** 1.2. **Fecha:** 16 de septiembre de 2026 (v1.0 borrador: 15-sep; v1.1: 16-sep). **Estado:** aprobado por Walter el 16-sep-2026; v1.2 incorpora la dirección visual de Codex y la historia recortada a 3 minutos (ver §14). Hosting pendiente (decisión 10). No hay código construido.
...
- RF-47. Spike de 1 día (semana 2) con el servidor de ejemplo del puente: cuenta Amazon Developer, ASK CLI, CDK, AgentCore, Nova 2 Lite, simulador de la consola. Salida: "responde" o "documentado por qué no".
- RF-48. Integración completa (semana 5, tope 3 días) contra el servidor alojado; sobre el puente, escritura verificada síncrona (sus timeouts son largos); rótulo "Alexa Skill simulator via alexa-skill-mcp-bridge". Si el agente del puente sobreafirma sobre un `PENDING`, se graba: es evidencia de la tesis y entrada de friction log.
...
| `PENDING` contradice el modelo de Alexa+. | Ruta rápida síncrona cuando cabe (P1); tabla p50/p95; frase solo-voz; feedback a Amazon. |
